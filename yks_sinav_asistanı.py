...
import os
import matplotlib
matplotlib.use('TkAgg') # Matplotlib'in Tkinter backend'ini kullanması için ayar
...
# Uygulama genelinde kullanılan renkler, konu listeleri ve metin sabitleri
# bu sınıfta toplanarak merkezi bir yönetim sağlanmıştır.
# =============================================================================
class UygulamaAyarlari:
    """
    Uygulama genelindeki sabit verileri ve konfigürasyon ayarlarını tutar.
    Encapsulation (Kapsülleme) prensibi gereği veriler tek bir noktadan yönetilir.
    """
    RENKLER = {
        "BG": "#f4f7f6",
        "SIDEBAR": "#2c3e50",
        ...
        "TEXT": "#2c3e50",
        "ACCENT": "#2980b9",
        "LOGIN_BG": "#34495e" # Giriş ekranı arkaplan rengi
    }

    # TYT ve AYT dersleri ve alt konularını içeren veri havuzu (Dictionary yapısı)
    KONU_HAVUZU = {
        "TYT": {
...
class VeriTabaniYoneticisi:
    """
    SQLite veritabanı ile iletişim kuran, tablo oluşturan ve
    CRUD (Create, Read, Update, Delete) işlemlerini yöneten sınıf.
    """
    def __init__(self):
        # Veritabanı dosyasının yolunu dinamik olarak belirler
    ...
    def _tablo_olustur(self):
        """Deneme sonuçlarının saklanacağı ana tabloyu oluşturur."""
    ...
    def veri_ekle(self, veriler):
        """Arayüzden gelen deneme sonuçlarını veritabanına kaydeder."""
    ...
    def tum_verileri_al(self):
        """Veritabanındaki tüm deneme sonuçlarını Pandas DataFrame olarak döndürür."""
        return pd.read_sql_query("SELECT * FROM denemeler", self.conn)

# =============================================================================
# 3. İŞ MANTIĞI VE ANALİZ KATMANI (Business Logic & OOP)
# Soyut Sınıf (Abstract Class) ve Kalıtım (Inheritance) kullanımı
# =============================================================================
class AnalizMotoru(ABC):
    """
    Analiz işlemleri için temel soyut sınıf.
    Polymorphism (Çok Biçimlilik) için bir arayüz sağlar.
    """
    @abstractmethod
...
class DersAnalizcisi(AnalizMotoru):
    """
    Tekil ders bazında netleri analiz eden ve seviyeye göre yorum üreten sınıf.
    """
    def analiz_yap(self, ders_adi, net, soru_sayisi):
        try:
            ...
            # Başarı yüzdesine göre seviye belirleme (0-3 arası)
            ...
            # İlgili dersin yorum setini seç ve rastgele bir yorum döndür
...
class ProgramOlusturucu(AnalizMotoru):
    """
    Kullanıcının seviyesine ve eksik konularına göre 
    haftalık ders çalışma programı oluşturan sınıf.
    """
    def analiz_yap(self, seviye_id, eksik_konular_str):
        gunler = ["Pazartesi", "Salı", "Çarşamba", "Perşembe", "Cuma", "Cumartesi", "Pazar"]
        
        # Eksik konular string verisini listeye çevirme
        ...
        # Eğer eksik konu seçilmediyse varsayılan bir liste kullan
        if not eksik_listesi:
            eksik_listesi = ["TYT Mat", "AYT Fizik", "AYT Kimya", "Geometri", "TYT Türkçe", "Tarih", "Coğrafya"]

        # Seviyeye göre farklı çalışma rutinleri tanımlanması
        sabah_rutinleri = {
        ...
        program = []
        random.shuffle(eksik_listesi) # Konuları günlere dağıtmak için karıştır
...
class KararDestekSistemi(AnalizMotoru):
    """
    Kullanıcının genel durumunu yapay zeka (Karar Ağacı) ile analiz eden
    ve diğer analiz sınıflarını koordine eden ana yönetici sınıf.
    """
    def __init__(self):
    ...
    def analiz_yap(self, tyt_net, ayt_net, eksik_konular_str, turk_net, mat_net, fen_net, sos_net):
        try:
            # Yapay Zeka (Decision Tree) Eğitimi
            # Basit bir veri seti ile öğrenci seviyesini sınıflandırma
            X_train = [[20, 5], [45, 15], [65, 30], [85, 50], [105, 70]] # Girdi: [TYT, AYT]
            y_train = [0, 1, 2, 3, 4] # Çıktı: Seviye ID'leri (0: Başlangıç, 4: Derece)
            
            clf = DecisionTreeClassifier()
            clf.fit(X_train, y_train)
            sinif_id = clf.predict([[tyt_net, ayt_net]])[0] # Mevcut netlere göre tahmin
            
            hedef_net = tyt_net + 5.5 
            
            # Alt analiz modüllerinin çağrılması
            ...
            # Seviyeye göre dinamik rapor içeriği
            zengin_yorumlar = {
            ...
            # Haftalık programın oluşturulması
...
# Tkinter kütüphanesi kullanılarak oluşturulan GUI katmanı.
# =============================================================================
class YKSAsistaniUI:
    """
    Uygulamanın ana penceresini ve tüm görsel bileşenlerini yöneten sınıf.
    """
    def __init__(self, root):
        ...
        # Bağlantılı sınıfların başlatılması
        self.db_yoneticisi = VeriTabaniYoneticisi()
        self.karar_sistemi = KararDestekSistemi()
        
        # Kullanıcı Adı değişkeni (Giriş ekranından alınacak)
    ...
    def _giris_ekrani_olustur(self):
        """Uygulama açılışında gelen karşılama ve isim alma ekranı."""
        ...
        # Rastgele Motivasyon Sözü Gösterimi
    ...
    def _giris_yap(self):
        """Giriş butonuna basılınca çalışır, ana arayüze geçişi sağlar."""
        ...
        self.giris_frame.destroy() # Giriş ekranını bellekten sil
        self._arayuz_olustur() # Ana arayüzü kur
        self.sayfa_analiz(None)

    def _arayuz_olustur(self):
        """Ana uygulama düzenini (Sidebar ve İçerik Alanı) oluşturur."""
    ...
    def _stil_ayarla(self):
        """Tkinter widget'ları için genel stil tanımlamaları."""
        style = ttk.Style()
    ...
    def temizle_content(self):
        """İçerik alanındaki tüm widget'ları temizler (Sayfa geçişleri için)."""
        for widget in self.content_area.winfo_children():
            widget.destroy()

    def sayfa_yeni_giris(self):
        """Yeni deneme verilerinin girileceği formu oluşturur."""
        ...
        # Dinamik entry oluşturma ve sözlüğe kaydetme
        self.entry_dict = {}
    ...
    def _create_scrollable_area(self, parent_tab, konu_data):
        """Konu seçimi için kaydırılabilir (Scrollable) alan oluşturur."""
    ...
    def verileri_kaydet(self):
        """Formdaki verileri toplar, doğrular ve veritabanına gönderir."""
        try:
            ...
            for kod in ders_kodlari:
                ...
                # Boş bırakılan yerleri 0 olarak kabul et
    ...
    def sayfa_analiz(self, secilen_indeks=None):
        """Analiz sonuçlarını gösteren dashboard ekranını oluşturur."""
        self.temizle_content()
        ...
        # Basit puan hesaplama (yaklaşık katsayılar)
        ...
        # Karar Destek Sistemi üzerinden analiz ve program oluşturma
    ...
    def _create_card(self, parent, title, value, color, col_idx):
        """Dashboard'daki bilgi kartlarını oluşturur."""
